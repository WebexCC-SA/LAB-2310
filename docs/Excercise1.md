...
The objective of this lab exercise is to enable participants to:

Part 1: Discovering Tenant Call Flow Configuration

Part 2: Configre and Debug an Inbound Flow
Set up an inbound flow that includes a whisper announcement capability and utilizes a "Team" and "Longest Available" distribution strategy for agent assignment.
During this setup, identify and resolve common misconfigurations by leveraging the flow debugger capabilities.

Part 3: Analyze Agent Performance with Call Reports
After successfully setting up the inbound flow and presenting calls to a test agent, intentionally reject one call and trigger a RONA (Ring No Answer) event with a second call. This will allow for a step-by-step exploration of reports that can be used to analyze agent behavior and identify the cause of the rejected and RONA calls.
...
- There are two reports in the folder which are copy of the stock report: "Agent Trace Report" and "Queue Activity By Queue Report." Each report includes three columns:

- RONA Count: This captures all calls that failed because the configured RONA timer expired in WxCC (reasonCode: RONA_TIMER_EXPIRED) or 
               Because of a configured ring timeout on the device (reasonCode: NO_ANSWER_USER). 
...
Congratulations ! on completing this section of the lab! You've successfully navigated the complexities of call flow configuration, debugged common issues, and learned how to analyze agent performance by simulating and tracking different call outcomes in reports.
